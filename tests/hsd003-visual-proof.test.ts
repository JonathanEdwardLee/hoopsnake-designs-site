...
describe('HSD-003 visual proof and approved logo', () => {
  ...
  const featuredChunk = indexSource.slice(
    ...
    indexSource.indexOf('id="more-work"'),
  );

  it('sends Portfolio navigation to the first proof section while keeping More Work anchored', () => {
    expect(layoutSource).toContain('href="#portfolio">Portfolio');
    expect(indexSource).toContain('id="portfolio"');
    expect(indexSource).toContain('id="more-work"');
    expect(indexSource).toContain('<h2 id="more-work-title" class="section-title">More Work</h2>');
  });

  it('follows the Portfolio label with Orpheus visual proof, not a title block', () => {
    expect(featuredChunk).toContain('>Portfolio<');
    expect(featuredChunk).not.toContain('>Featured<');
  ...
  it('uses current Play Store/app screenshots and the Junkfeathers art-project label', () => {
    expect(existsSync(resolve(root, 'public/images/portfolio/orpheus-play-main.webp'))).toBe(true);
    expect(existsSync(resolve(root, 'public/images/portfolio/orpheus-play-recording.webp'))).toBe(true);
    expect(existsSync(resolve(root, 'public/images/portfolio/orpheus-play-chronos.webp'))).toBe(true);
